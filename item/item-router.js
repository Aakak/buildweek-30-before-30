...
router.get('/:id/items/:item_id', Middleware.setUserId, Middleware.getBucketList, getItem, getComments, (req, res) => {
    const item = req.params.item;
    item.comments = req.params.comments || [];
    res.status(200).json(item);
});

router.put('/:id/items/:item_id', Middleware.setUserId, Middleware.getBucketList, getItemPrivate, (req, res) => {
  const item = req.body;
...
router.post('/:id/items', Middleware.setUserId, Middleware.getBucketList, (req, res) => {
  const item = req.body;
  const bucketlist = req.params.bucketlist;
  item.bucketlist_id = bucketlist.id;
  Items.add(item).then(item =>
    res.status(200).json(item)
  ).catch(error => {
    console.log(error)
...
router.delete('/:id/items/:item_id', Middleware.setUserId, Middleware.getBucketList, getItemPrivate, (req, res) => {
  const itemId = req.params.item.id;
  Items.remove(itemId).then(ids =>
...
function getComments(req, res, next) {
  if (req.params.item) {
    Comments.findBy({"item_id": req.params.item.id}).then(comments =>{
